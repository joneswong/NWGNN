...
class MixAggrConvLayer(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, improved=False, cached=False,
        ...
        self._cand_aggrs = kwargs.pop('cand_aggrs', None)
        super(MixAggrConvLayer, self).__init__(flow=cfg.gnn.flow, **kwargs)
        ...
        #self.weights = Parameter(torch.Tensor(in_channels, out_channels))
    ...
    def aggregate(self, inputs: Tensor, index: Tensor,
        ...
        if ptr is not None:
            ...
            for i, aggr in enumerate(self._cand_aggrs):
                ...
                #results += self._aggr_weight[:,i:i+1] * scatter(inputs, index, dim=self.node_dim, dim_size=dim_size,
                #           reduce=self._cand_aggrs[i])
            return results
...
class MixAggrConv(nn.Module):

    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        super(MixAggrConv, self).__init__()
        self.model = MixAggrConvLayer(dim_in, dim_out, bias=bias, **kwargs)

    def forward(self, batch, aggr_weight=None):
        batch.x = self.model(batch.x, batch.edge_index, aggr_weight=aggr_weight)
        return batch


register_layer('mixaggrconv', MixAggrConv)
